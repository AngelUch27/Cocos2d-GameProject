...
Scene* JuegoFacil::createScene()
{
    Scene* scene = Scene::createWithPhysics();
    scene->getPhysicsWorld()->setGravity(Vect(0, 0));
    scene->getPhysicsWorld()->setDebugDrawMask(PhysicsWorld::DEBUGDRAW_ALL);
    scene->addChild(JuegoFacil::create());
...
bool JuegoFacil::init()
{
    ...
    auto SpriteMenuFondo = Sprite::create("FondoTierra.png");
    SpriteMenuFondo->setAnchorPoint(Vec2::ZERO);
    /*SpriteMenuFondo->setScale(1.20);
    SpriteMenuFondo->setRotation(90);
    SpriteMenuFondo->setPosition(400, 400);
    */
    SpriteMenuFondo->setScale(1.75);
    ...
    //Audio
    auto audio = AudioEngine::play2d("Level1.mp3", true);
   ...
   // auto action = Sequence::create(RotateBy::create(2, 100), MoveBy::create(5, Point(100, -50)), NULL);
    ...
    //Teclado
    listenerKeyboard->onKeyPressed = CC_CALLBACK_2(JuegoFacil::onKeyPressed, this);
    listenerKeyboard->onKeyReleased = CC_CALLBACK_2(JuegoFacil::onKeyReleased, this);
    ...
    contactListener->onContactBegin = CC_CALLBACK_1(JuegoFacil::onContactBegin, this);
    ...
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoFacil::funcionNaves),3.0f);
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoFacil::funcionGas), 17.0f);
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoFacil::meChocaron), .1);
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoFacil::checador), .2);
...
void JuegoFacil::checador(float Delta)
{
...
//Trabaja solo
bool JuegoFacil::onContactBegin(PhysicsContact& contact)
{
...
//2 Enemigos
//3Monedas
void JuegoFacil::meChocaron(float delta)
{
    if (choqueC)
    {
...
//Monedas
void JuegoFacil::funcionGas(float delta)
{
    agarramela = rand() % 450;
...
void JuegoFacil::funcionNaves(float delta2)
{
    ...
    auto avanzarOvnis = MoveBy::create(4, Vec2(0, -1100));
...
void JuegoFacil::onKeyPressed(cocos2d::EventKeyboard::KeyCode key, cocos2d::Event* event)
{
...
void JuegoFacil::onKeyReleased(cocos2d::EventKeyboard::KeyCode key, cocos2d::Event* event)
{
    if (key == EventKeyboard::KeyCode::KEY_DOWN_ARROW)
    {
        //CCLOG("Key A pressed");
...
//Enemigo1
void JuegoFacil::update(float deltaTime)
{
    ...
    void JuegoFacil::menuCloseCallback(Ref * pSender)
    {
