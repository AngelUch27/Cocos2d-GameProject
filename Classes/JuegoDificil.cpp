...
Scene* JuegoDificil::createScene()
{
    Scene* scene = Scene::createWithPhysics();
    scene->getPhysicsWorld()->setGravity(Vect(0, 0));
    scene->getPhysicsWorld()->setDebugDrawMask(PhysicsWorld::DEBUGDRAW_ALL);
    scene->addChild(JuegoDificil::create());
...
bool JuegoDificil::init()
{
    ...
    auto SpriteMenuFondo = Sprite::create("Fondo3.png");
    ...
    //Audio
    auto audio = AudioEngine::play2d("Level3.mp3", true);
    ...
    //Teclado
    listenerKeyboard->onKeyPressed = CC_CALLBACK_2(JuegoDificil::onKeyPressed, this);
    listenerKeyboard->onKeyReleased = CC_CALLBACK_2(JuegoDificil::onKeyReleased, this);
    ...
    contactListener->onContactBegin = CC_CALLBACK_1(JuegoDificil::onContactBegin, this);
    ...
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoDificil::funcionNaves), 1.0f);
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoDificil::funcionGas), 17.0f);
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoDificil::meChocaron), .1);
    this->schedule(CC_SCHEDULE_SELECTOR(JuegoDificil::checador), .2);
...
void JuegoDificil::checador(float Delta)
{
...
//Trabaja solo
bool JuegoDificil::onContactBegin(PhysicsContact& contact)
{
...
//2 Enemigos
//3Monedas
void JuegoDificil::meChocaron(float delta)
{
    if (choqueC)
    {
    ...
    {
        cocos2d::AudioEngine::stopAll();
        auto VicScene = Victory::createScene();
        Director::getInstance()->replaceScene(VicScene);
    }
    if (litrosGas == 2)
    {
...
//Monedas
void JuegoDificil::funcionGas(float delta)
{
    agarramela = rand() % 450;
...
void JuegoDificil::funcionNaves(float delta2)
{
    ...
    auto avanzarOvnis = MoveBy::create(1.5, Vec2(0, -1100));
...
void JuegoDificil::onKeyPressed(cocos2d::EventKeyboard::KeyCode key, cocos2d::Event* event)
{
...
void JuegoDificil::onKeyReleased(cocos2d::EventKeyboard::KeyCode key, cocos2d::Event* event)
{
    if (key == EventKeyboard::KeyCode::KEY_DOWN_ARROW)
    {
        //CCLOG("Key A pressed");
...
//Enemigo1
void JuegoDificil::update(float deltaTime)
{
...
void JuegoDificil::menuCloseCallback(Ref* pSender)
{
